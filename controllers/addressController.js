...
module.exports = {
    ...
    setAddressDefault: async (req, res) => {
        ...
        try {
            ...
            if (updateAddress) {
                ...
                res.status(200).json({ status: true, message: "Address successfully set as defalut" });
            } else {
    ...
    getDefaultAddress: async (req, res) => {
        ...
        try {
            const address = await Address.findOne({ userId: userId, default: true }, { __v: 0 });

            res.status(200).json(address)
        } catch (error) {
            res.status(500).json({ status: false, message: "dwiahdowuian" + error.message })
        }
    }
}